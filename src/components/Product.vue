...
export default {
    setup() {
        ...
        const allDate = reactive({data: {}});  // 儲存原始的所有資料
        ...
        if (isLogin.value) {
            // 已登入才會繼承localStorage裡的資料
            myFavorite.value = favoriteData.value;

            // 若 favoriteData 為null，則 myFavorite 設定空陣列
            ...
            <ul class="product-box" >
                <li v-for="(item, idx) in product.data" :key="item.PID">
                    <div class="img-box">
                        ...
                        <svg v-show="myFavorite.includes(item.PID)" @click="removeLike(idx, item.PID)" 
